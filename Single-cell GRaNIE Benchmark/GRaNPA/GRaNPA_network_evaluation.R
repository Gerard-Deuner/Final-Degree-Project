...
cpl <- GRaNPA::plot_GRaNPA_boxplot(GRaNPA.object_list = list(GRaNPA_d4d0res0.1, GRaNPA_d4d0res0.25, GRaNPA_d4d0res0.5, GRaNPA_d4d0res0.75,
                                   ...
                                   plot_name = paste0("Resolutions_Comparison_GRaNPA_d4d0.pdf"),
...
num_pred_genes <- c()
GRaNPA_list <- list(GRaNPA_d4d0res0.1, GRaNPA_d4d0res0.25, GRaNPA_d4d0res0.5, GRaNPA_d4d0res0.75,
                    GRaNPA_d4d0res1, GRaNPA_d4d0res2, GRaNPA_d4d0res3, GRaNPA_d4d0res4,
                    GRaNPA_d4d0res5, GRaNPA_d4d0res6, GRaNPA_d4d0res7, GRaNPA_d4d0res8,
                    GRaNPA_d4d0res9, GRaNPA_d4d0res10, GRaNPA_d4d0res12, GRaNPA_d4d0res14,
                    GRaNPA_d4d0res16, GRaNPA_d4d0res18, GRaNPA_d4d0res20)

for (GRaNPA.obj in GRaNPA_list){
  num <- GRaNPA.obj$normal_models[[1]]$finalModel$predictions %>% length
  num_pred_genes <- c(num_pred_genes, num)
}

df <- data.frame(num = num_pred_genes, res = as.factor(resolutions))

bp <- ggplot(df, aes(x = res, y = num)) + 
  geom_bar(stat = "identity", col = "black") + 
  labs(x = "Resolutions", y  = "Number of Predicted Genes") + 
  theme_classic()

ggsave(paste0("/g/scb/zaugg/deuner/GRaNPA/figures/resolutions_eGRNs_comparison_d4d0_round", round,".png"), cpl, device = "png")
ggsave(paste0("/g/scb/zaugg/deuner/figs/Number_predicted_genes.png"), bp, device = "png", dpi = 300)
...
# also save GRaNPA object
qsave(GRaNPA_d4d0, file = "/g/scb/zaugg/deuner/GRaNPA/outputdata/NPC_NEURON_d4d0_GRaNPA_obj.qs")
...
# Investigate important TFs
p3 <- GRaNPA::plot_GRaNPA_TF_imp(GRaNPA.object = GRaNPA_d4d0, plot_name = "NPC_NEURON_Day4vsDay2.pdf", output = "/g/scb/zaugg/deuner/GRaNPA/figures", width = 4, height = 4)

pl <- ggarrange(p1, p2, p3, labels = c("A", "B", "C")) %>%
  annotate_figure(top = text_grob("GRaNPA Analysis - Day 4 vs. Day 2",
                                  color = "black", face = "bold", size = 14))
pl
ggsave("/g/scb/zaugg/deuner/GRaNPA/figures/NPC_NEURON_Day4vsDay2.png", pl, device = "png")

# also save GRaNPA object
qsave(GRaNPA_d4d0, file = "/g/scb/zaugg/deuner/GRaNPA/outputdata/NPC_NEURON_d4d2_GRaNPA_obj.qs")
...
# Investigate important TFs
p3 <- GRaNPA::plot_GRaNPA_TF_imp(GRaNPA.object = GRaNPA_d4d0, plot_name = "NPC_NEURON_Day2vsDay0.pdf", output = "/g/scb/zaugg/deuner/GRaNPA/figures", width = 4, height = 4) 

pl <- ggarrange(p1, p2, p3, labels = c("A", "B", "C")) %>% 
  annotate_figure(top = text_grob("GRaNPA Analysis - Day 2 vs. Day 0", 
                                  color = "black", face = "bold", size = 14))
pl
ggsave("/g/scb/zaugg/deuner/GRaNPA/figures/NPC_NEURON_Day2vsDay0.png", pl, device = "png")

# also save GRaNPA object
qsave(GRaNPA_d4d0, file = "/g/scb/zaugg/deuner/GRaNPA/outputdata/NPC_NEURON_d2d0_GRaNPA_obj.qs")
...
for (GRaNPA in GRaNPA_objects){
  TF_names <- GRaNPA$nrm_imp_scaled %>%
...
i <- 1
for (GRaNPA in GRaNPA_objects){
  TF_scores <- GRaNPA$nrm_imp_scaled %>%
    arrange(desc(Overall))
...
names(imp_TFs.df) <- as.factor(names(imp_TFs.df))
imp_TFs.df$TF <- rownames(imp_TFs.df)
imp_TFs.df.long <- pivot_longer(imp_TFs.df, cols=as.factor(resolutions), names_to = "resolution", values_to = "score")
...
# reorder df
imp_TFs.df.long$resolution <- factor(imp_TFs.df.long$resolution, levels = resolutions)
...
# heatmap
ggplot(imp_TFs.df.long, aes(x = resolution, y = TF, fill = score)) + 
  geom_tile() + 
  scale_fill_viridis_c() + 
  labs(x = "Resolutions", y = "TFs")

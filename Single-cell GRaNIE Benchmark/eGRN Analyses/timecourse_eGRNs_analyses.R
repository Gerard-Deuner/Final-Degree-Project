...
path <- "/g/scb/zaugg/deuner/GRaNIE/outputdata/batch_mode/timecourse_batch_mode_spearman_nomicro/Batch_Mode_Outputs/"
...
for (res in resolutions){
  ...
  GRN.uniqueTFs <- GRN@connections$all.filtered$`0` %>% distinct(TF.ID)
  GRN.uniqueTFs.len <- GRN.uniqueTFs %>% nrow()
  metrics <- c(metrics, GRN.uniqueTFs.len)
  GRN.uniquePEAKs <- GRN@connections$all.filtered$`0` %>% distinct(peak.ID)
  GRN.uniquePEAKs.len <- GRN.uniquePEAKs %>% nrow()
  metrics <- c(metrics, GRN.uniquePEAKs.len)
  GRN.uniqueGENEs <- GRN@connections$all.filtered$`0` %>% distinct(gene.ENSEMBL)
  GRN.uniqueGENEs.len <- GRN.uniqueGENEs %>% nrow()
  metrics <- c(metrics, GRN.uniqueGENEs.len)
  GRN.TF_peak.links <- GRN@connections$all.filtered$`0` %>% distinct(TF.ID, peak.ID) 
  GRN.TF_peak.links.len <- GRN.TF_peak.links %>% nrow()
  metrics <- c(metrics, GRN.TF_peak.links.len)
  GRN.peak_gene.links <- GRN@connections$all.filtered$`0` %>% distinct(gene.ENSEMBL, peak.ID)
  GRN.peak_gene.links.len <- GRN.peak_gene.links%>% nrow()
  metrics <- c(metrics, GRN.peak_gene.links.len)
  GRN.TF_peak_gene.links <- GRN@connections$all.filtered$`0` %>% distinct(TF.ID, peak.ID, gene.ENSEMBL) 

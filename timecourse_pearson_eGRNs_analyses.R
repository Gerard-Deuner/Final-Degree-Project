#####################################
# TIMECOURSE-PEARSON eGRNs ANALYSES #
#####################################
...
for (res in resolutions){
  ...
  # significant peak-gene links (p < 0.05)
  GRN.sig.peak_gene.links <- GRN@connections$peak_genes$`0` %>% dplyr::filter(peak_gene.p_raw < 0.05)
  ...
  assign(paste("SIG.PEAK.GENE", res, sep = "."), GRN.sig.peak_gene.links)
...
# create binary df with all significant peak-genes links and resolutions as columns
df_list2 <- list(SIG.PEAK.GENE.0.1, SIG.PEAK.GENE.0.25, SIG.PEAK.GENE.0.5, SIG.PEAK.GENE.0.75, SIG.PEAK.GENE.1, SIG.PEAK.GENE.2, 
                 SIG.PEAK.GENE.3, SIG.PEAK.GENE.4, SIG.PEAK.GENE.5, SIG.PEAK.GENE.6, SIG.PEAK.GENE.7, SIG.PEAK.GENE.8, SIG.PEAK.GENE.9, 
                 SIG.PEAK.GENE.10, SIG.PEAK.GENE.12, SIG.PEAK.GENE.14, SIG.PEAK.GENE.16, SIG.PEAK.GENE.18, SIG.PEAK.GENE.20)
...
tc.links <- tc.links %>% inner_join(annotLookup, by = "gene", multiple = "all") 
colnames(tc.links)[14] <- "gene.ENSEMBL"
head(annotLookup)
head(tc.links)
...
for (res in resolutions){
  ...
  GRN_links <- GRN@connections$peak_genes$`0` %>% dplyr::filter(peak_gene.p_raw < 0.05) %>% as.data.frame() %>% dplyr::select(peak.ID, gene.ENSEMBL, peak_gene.r, peak_gene.p_raw)
  ...
  names(GRN_links)[1:2] <- c("peak", "gene.ENSEMBL")
  
  # Binary classification
  # get rows that match in GRN links and HiC links by gene name and peak id
  TP <- inner_join(GRN_links, tc.links, by = c("peak", "gene.ENSEMBL"), multiple = "all")
  ...
  if ((first == TRUE) & (sum(bc) > 0)){
    ROC <- roc(GRN_links$bc, GRN_links$peak_gene.r, plot = TRUE, legacy.axes = TRUE, percent = TRUE,
